<?php
		...
		$perm_incluir = testarPermissao('INCLUIR CADASTRO DE TIPOS DE DOCUMENTO');
		$perm_alterar = testarPermissao('ALTERAR CADASTRO DE TIPOS DE DOCUMENTO');
		$perm_excluir = testarPermissao('EXCLUIR CADASTRO DE TIPOS DE DOCUMENTO');
	...
	<body>
		<?php
			...
			if (!empty($id)) {
				...
				$sql = "select * from tipos_documento where id=" . $id;
				...
				<div class="row">
					...
					<div id="conteudo" class="col-xs-12 col-md-10">
								...
								<form role="form">
									<div class="row">
									    ...
									    <div class="form-group col-md-6">
										    <label for="descricao">Nome do Documento: <span class="label label-danger">Obrigatório</span></label>
						...
						<div class="aviso">
							<?php
								if ($_action == 'inclusao' && $perm_incluir != 'S') {
									echo "<script>avisoAtencao('Sem permissão: INCLUIR CADASTRO DE TIPOS DE DOCUMENTO. Solicite ao administrador a liberação.');</script>";
								}
								
								if ($_action == 'alteracao' && $perm_alterar != 'S') {
									echo "<script>avisoAtencao('Sem permissão: ALTERAR CADASTRO DE TIPOS DE DOCUMENTO. Solicite ao administrador a liberação.');</script>";
								}
							?>
						</div>
						<!-- PAINEL DE BOTOES -->
						<div class="btn-control-bar">
							<div class="panel-heading">
								...
								<button class="btn btn-danger mob-btn-block" style="<?php if ($_action == "inclusao") { echo "display: none"; } ?>" data-toggle="modal" data-target="#modal" onclick="dialogYesNo('esubmit()', null, 'Excluir Tipo de Documento', 'Deseja excluir este Tipo de Documento ?', 'trash');" <?php if ($perm_excluir != 'S') { echo "disabled"; } ?>>
