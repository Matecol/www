<?php
		...
		$perm_incluir = testarPermissao('INCLUIR CADASTRO DE LINHAS');
		$perm_alterar = testarPermissao('ALTERAR CADASTRO DE LINHAS');
		$perm_excluir = testarPermissao('EXCLUIR CADASTRO DE LINHAS');
	...
	<body>
		<?php
			...
			if (!empty($id)) {
				...
				$sql = "select * from linhas where id=" . $id;
				...
				<div class="row">
					...
					<div id="conteudo" class="col-xs-12 col-md-10">
								...
								<form role="form">
									<div class="row">
							    		...
							    		<div class="form-group col-md-6">
										    ...
										    <input type="text" class="form-control" id="linha" name="linha" autocomplete="off" maxlength="60" value="<?= $linha ?>" autofocus <?php permissao(); ?> required>
									    </div>
									</div>
						...
						<div class="aviso">
							<?php
								if ($_action == 'inclusao' && $perm_incluir != 'S') {
									echo "<script>avisoAtencao('Sem permissão: INCLUIR CADASTRO DE LINHAS. Solicite ao administrador a liberação.');</script>";
								}
								
								if ($_action == 'alteracao' && $perm_alterar != 'S') {
									echo "<script>avisoAtencao('Sem permissão: ALTERAR CADASTRO DE LINHAS. Solicite ao administrador a liberação.');</script>";
								}
							?>
						</div>
						<!-- PAINEL DE BOTOES -->
						<div class="btn-control-bar">
							<div class="panel-heading">
								...
								<button class="btn btn-danger mob-btn-block" style="<?php if ($_action == "inclusao") { echo "display: none"; } ?>" data-toggle="modal" data-target="#modal" onclick="dialogYesNo('esubmit()', null, 'Excluir Linha', 'Deseja excluir esta Linha ?', 'trash');" <?php if ($perm_excluir != 'S') { echo "disabled"; } ?>>
