<?php
		...
		$perm_incluir = testarPermissao('INCLUIR CADASTRO DE SUBGRUPOS');
		$perm_alterar = testarPermissao('ALTERAR CADASTRO DE SUBGRUPOS');
		$perm_excluir = testarPermissao('EXCLUIR CADASTRO DE SUBGRUPOS');
	...
	<body>
		<?php
			...
			if (!empty($id)) {
				...
				$sql = "select * from subgrupos where id=" . $id;
				...
				<div class="row">
					...
					<div id="conteudo" class="col-xs-12 col-md-10">
						...
						<div class="aviso">
							<?php
								if ($_action == 'inclusao' && $perm_incluir != 'S') {
									echo "<script>avisoAtencao('Sem permissão: INCLUIR CADASTRO DE SUBGRUPOS. Solicite ao administrador a liberação.');</script>";
								}
								
								if ($_action == 'alteracao' && $perm_alterar != 'S') {
									echo "<script>avisoAtencao('Sem permissão: ALTERAR CADASTRO DE SUBGRUPOS. Solicite ao administrador a liberação.');</script>";
								}
							?>
						</div>
						<!-- PAINEL DE BOTOES -->
						<div class="btn-control-bar">
							<div class="panel-heading">
								...
								<button class="btn btn-danger mob-btn-block" style="<?php if ($_action == "inclusao") { echo "display: none"; } ?>" data-toggle="modal" data-target="#modal" onclick="dialogYesNo('esubmit()', null, 'Excluir Subgrupo', 'Deseja excluir este Subgrupo ?', 'trash');" <?php if ($perm_excluir != 'S') { echo "disabled"; } ?>>
