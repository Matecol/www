<?php
		...
		$perm = testarPermissao('INCLUIR CADASTRO DE CONTAS DO MOVIMENTO FINANCEIRO');
					...
					<div id="conteudo" class="col-xs-12 col-md-10">
						...
						<div class="panel panel-primary">
							<div class="panel-heading">
								Consulta de Contas do Movimento Financeiro
							</div>
							<div class="panel-body">
								...
								<table class="table table-hover table-striped tabela-registro" id="tabela">
									...
									<?php
										...
										if (empty($pesquisa)) {
											$sql = "select * from contas_movimento_financeiro order by nome limit " . $limite . " offset " . (($pagina-1)*$limite);
										} else {
											$sql = "select * from contas_movimento_financeiro where nome like '" . $pesquisa . "%' order by nome limit " . $limite . " offset " . (($pagina-1)*$limite);
										}
										...
										// Paginaçao
										if (empty($pesquisa)) {
											$sql = "select count(*) as num from contas_movimento_financeiro";
										} else {
											$sql = "select count(*) as num from contas_movimento_financeiro where nome like '" . $pesquisa . "%';";
										}
						...
						<div class="aviso">
							<?php
								if ($perm != 'S') {
									echo "<script>avisoAtencao('Sem permissão: INCLUIR CADASTRO DE CONTAS DO MOVIMENTO FINANCEIRO . Solicite ao administrador a liberação.');</script>";
								}
							?>
						</div>
						<!-- PAINEL DE BOTOES -->
